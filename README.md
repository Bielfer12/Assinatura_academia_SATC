...
* [Script de Criação do Banco de Dados](scripts/ddl/database_creation.sql) (Se houver um script separado para o `CREATE DATABASE` e `USE [ABP]`)
* [Tabelas](scripts/ddl/tabelas)
* [Índices](scripts/ddl/indices) - Desenvolvidos para otimizar o desempenho das consultas, com foco em índices não agrupados como `IX_ControleAcessos_Cliente_DataEntrada` e `IX_Contratos_Cliente_Status_Periodo`. A escolha e a eficácia desses índices foram validadas através da análise do plano de acesso, operadores e chaves das consultas.
* [Gatilhos (Triggers)](scripts/ddl/gatilhos) - Implementado, por exemplo, um gatilho para registrar a `dt_ultima_inativacao` na tabela `Contratos` automaticamente quando o status de um contrato é alterado para 'INATIVO', garantindo a consistência do histórico de dados.
...
* [Funções](scripts/ddl/funcoes) - Inclui a função escalar `dbo.fn_ObterNivelAtividadeCliente`, que calcula o nível de atividade de um cliente (Muito Ativo, Ativo, Moderado, Pouco Ativo, Inativo) com base na quantidade de acessos em um dado período e o status do contrato. Esta função demonstra a capacidade de encapsular lógica de negócio reutilizável diretamente no banco de dados.
* [DML (População de Dados)](scripts/dml) - Scripts para popular as tabelas com dados de exemplo (mínimo de 10 itens principais por tabela relevante), facilitando o teste e a demonstração do sistema.
